...
export const CrossFiTransactionAnalyticsTool = new DynamicStructuredTool({
    ...
    func: async (input) => {
        try {
            ...
            const limitedBlockCount = Math.min(Math.max(blockCount, 1), 50);
...
export const CrossFiEcosystemSummaryTool = new DynamicStructuredTool({
    ...
    func: async () => {
        try {
            const [networkStats, tokenHolders, validators, staking, defiMetrics, governance] = await Promise.all([
...
//# sourceMappingURL=crypto-assistant.js.map