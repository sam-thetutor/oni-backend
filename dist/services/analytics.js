...
export class AnalyticsService {
    ...
    static async recordTransaction(userId, walletAddress, amount, token, type, transactionHash, status = 'completed') {
        try {
            ...
            userAnalytics.totalVolume[token.toLowerCase()] =
                this.addStrings(userAnalytics.totalVolume[token.toLowerCase()], amount);
            userAnalytics.lastActivity = new Date();
            await userAnalytics.save();
            const overview = await AnalyticsOverviewModel.findOne();
            if (overview) {
                overview.totalTransactions += 1;
                overview.totalVolumeProcessed[token.toLowerCase()] =
                    this.addStrings(overview.totalVolumeProcessed[token.toLowerCase()], amount);
    ...
    static async recordMessage(userId, walletAddress, messageType) {
        try {
    ...
    static async recordPaymentLink(userId, walletAddress, amount, token) {
        try {
            const overview = await AnalyticsOverviewModel.findOne();
            if (overview) {
                overview.totalPaymentLinks.count += 1;
                overview.totalPaymentLinks.totalAmount[token.toLowerCase()] =
                    this.addStrings(overview.totalPaymentLinks.totalAmount[token.toLowerCase()], amount);
                overview.lastUpdated = new Date();
                await overview.save();
            }
    ...
    static async recordDCAOrder(userId, walletAddress, amount, token) {
        try {
            const overview = await AnalyticsOverviewModel.findOne();
            if (overview) {
                overview.totalDCAOrders.count += 1;
                overview.totalDCAOrders.totalVolume[token.toLowerCase()] =
                    this.addStrings(overview.totalDCAOrders.totalVolume[token.toLowerCase()], amount);
                overview.lastUpdated = new Date();
                await overview.save();
            }
